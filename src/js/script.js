var myResult = {};
var section = ['A', 'Home', 'Opinion', 'World', 'National', 'Politics', 'Upshot', 'NyRegion', 'Business', 'Technology',
...
$(document).ready(function () {
  ...
  $('#menu').on('change', function () {
    var url = "https://api.nytimes.com/svc/topstories/v2/" + $('#menu').val().toLowerCase() + ".json";
    url += '?' + $.param({
  ...
  $('.carousel>figure').on('click',function(){
    if($(this).index() != 0 ){
      $('#menu').prop('selectedIndex', $(this).index()).selectric('refresh');
      $('#menu').val($(this).text()).trigger('change');
    }
  });
...
function createCarousel() {
  var tz = Math.round(($('.container').width() / 2) / Math.tan(Math.PI / section.length));
  for (var i = 0; i < section.length; i++) {
    $('.carousel').append('<figure>' + section[i] + '</figure>');
    degree = i * constDegree;
    $('.carousel').children().eq(i).css({
      'transform': 'rotateX(' + degree + 'deg) translateZ(' + tz + 'px) translateX(-150px)'
    });
  }
  $('.carousel').children().eq(0).addClass('chosen');
};
...
function highlight(text){
  ...
  $('.carousel').children().removeClass('chosen');
  $('.carousel').children().eq(index).addClass('chosen');
};
...
function createMenu(ary, theMenu) {
  ...
  for (var i = 0; i < ary.length; i++) {
    ...
    theMenu.children().last('option').attr({ 'value': ary[i] }).text(ary[i]);
  }
...
function createArticle(object) {
  ...
  for (i = 0; i < object.num_results; i++) {
    ...
    if (object.results[i].multimedia.length == 0) {
      ...
      $article.css({
        'background': 'url(' + object.results[i].multimedia[object.results[i].multimedia.length - 1].url + ') no-repeat center',
        'background-size': 'cover',
        'display': 'none',
        'animation-duration': '2s',
      });
  ...
  $('#display').children().each(function (index) {
    var element = $(this);
    setTimeout(function () {
